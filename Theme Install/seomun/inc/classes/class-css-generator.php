 ...
 * Convert HEX to GRBA
 */
if(!function_exists('seomun_rgba')){
    function seomun_rgba($hex,$opacity = 1) {
 ...
 */
function seomun_lighten( $hex, $percent ) {
...
class CSS_Generator
{
    ...
    function __construct() {
        $this->opt_name = seomun_get_opt_name();

        if ( empty( $this->opt_name ) ) {
            return;
        }
        $this->dev_mode = seomun_get_opt( 'dev_mode', '0' ) === '1' ? true : false;
    ...
    protected function options_output()
    {
        ...
        //Primary_color1
        $primary_color = seomun_get_opt( 'primary_color', '#ef3b58' );
        if ( ! seomun_is_valid_color( $primary_color ) )
        {
            $primary_color = '#ef3b58';
        }
        printf( '$primary_color: %s;', esc_attr( $primary_color ) );
        //Primary_color2
        $primary_color2 = seomun_get_opt( 'primary_color2', '#ffa61b' );
        if ( ! seomun_is_valid_color( $primary_color2 ) )
        {
            $primary_color2 = '#ffa61b';
        }
        printf( '$primary_color2: %s;', esc_attr( $primary_color2 ) );
        //Primary_color3
        $primary_color3 = seomun_get_opt( 'primary_color3', '#ffa61b' );
        if ( ! seomun_is_valid_color( $primary_color3 ) )
        {
            $primary_color3 = '#ffa61b';
        }
        printf( '$primary_color3: %s;', esc_attr( $primary_color3 ) );

        //Secondary Color 1
        $secondary_color = seomun_get_opt( 'secondary_color', '#7d8c95' );
        if ( ! seomun_is_valid_color( $secondary_color ) )
        {
            $secondary_color = '#7d8c95';
        }
        printf( '$secondary_color: %s;', esc_attr( $secondary_color ) );
        //Secondary Color 2
        $secondary_color2 = seomun_get_opt( 'secondary_color2', '#092232' );
        if ( ! seomun_is_valid_color( $secondary_color2 ) )
        {
            $secondary_color2 = '#092232';
        }
        printf( '$secondary_color2: %s;', esc_attr( $secondary_color2 ) );
        //Secondary Color 3
        $secondary_color3 = seomun_get_opt( 'secondary_color3', '#092232' );
        if ( ! seomun_is_valid_color( $secondary_color3 ) )
        {
            $secondary_color3 = '#092232';
        }
        printf( '$secondary_color3: %s;', esc_attr( $secondary_color3 ) );
        //Secondary Color 4
        $secondary_color4 = seomun_get_opt( 'secondary_color4', '#252525' );
        if ( ! seomun_is_valid_color( $secondary_color4 ) )
        {
            $secondary_color4 = '#252525';
        }
        printf( '$secondary_color4: %s;', esc_attr( $secondary_color4 ) );

        //Link Color
        $link_color = seomun_get_opt( 'link_color', '#ef3b58' );
        if ( !empty($link_color['regular']) && isset($link_color['regular']) )
        {
            printf( '$link_color: %s;', esc_attr( $link_color['regular'] ) );
        } else {
            echo '$link_color: #ef3b58;';
        }

        $link_color_hover = seomun_get_opt( 'link_color', '#7d8c95' );
        if ( !empty($link_color['hover']) && isset($link_color['hover']) )
        {
            printf( '$link_color_hover: %s;', esc_attr( $link_color['hover'] ) );
        } else {
            echo '$link_color_hover: #7d8c95;';
        }

        $link_color_active = seomun_get_opt( 'link_color', '#7d8c95' );
        if ( !empty($link_color['active']) && isset($link_color['active']) )
        {
            printf( '$link_color_active: %s;', esc_attr( $link_color['active'] ) );
        } else {
            echo '$link_color_active: #7d8c95;';
        }

        /* Gradient Color */
        $primary_color_gradient = seomun_get_opt( 'primary_color_gradient' );
    ...
    function enqueue()
    {
        ...
        if ( !empty($css) )
        {
            wp_add_inline_style( 'seomun-theme', $this->dev_mode ? $css : seomun_css_minifier( $css ) );
        }
    }
    ...
    protected function inline_css()
    {
        ob_start();
        /* BG Body */
        $body_background = seomun_get_opt( 'body_background' );
        ...
        /* Header */
        $h_bg_color = seomun_get_opt( 'h_bg_color' );
        $h_bg_top_color = seomun_get_opt( 'h_bg_top_color' );
        $h_text_top_color = seomun_get_opt( 'h_text_top_color' );
        $h_link_top_color = seomun_get_opt( 'h_link_top_color' );
        ...
        /* Logo */
        $logo_maxh = seomun_get_opt( 'logo_maxh' );
        ...
        $main_menu_color = seomun_get_opt( 'main_menu_color' );
        ...
        $main_menu_color_sub = seomun_get_opt( 'main_menu_color_sub' );
        ...
        /* Page Title */
        $ptitle_font_size = seomun_get_page_opt( 'ptitle_font_size' );
        ...
        /* Footer */
        $footer_bg = seomun_get_opt( 'footer_bg' );
        $footer_bg_color_top = seomun_get_opt( 'footer_bg_color_top' );
        $footer_top_heading_color = seomun_get_opt( 'footer_top_heading_color' );
        
        $footer_top_heading_fs = seomun_get_opt( 'footer_top_heading_fs' );
        $footer_top_paddings = seomun_get_opt( 'footer_top_paddings' );
        ...
        if(!empty($footer_bg_color_top)) {
            echo '.site-footer:before {
                background-color: '.esc_attr( $footer_bg_color_top['rgba'] ).';
            }';
        }
        if(!empty($footer_top_heading_color)) {
            echo '.top-footer .footer-widget-title {
                color: '.esc_attr( $footer_top_heading_color ).';
            }';
        }
        if(!empty($footer_top_heading_fs)) {
            echo '.top-footer .footer-widget-title {
                font-size: '.esc_attr( $footer_top_heading_fs ).'px;
            }';
        }
        ...
        /* Content */
        $post_text_align = seomun_get_opt( 'post_text_align', 'inherit' );
        if($post_text_align == 'justify') {
            echo '.single-post .content-area .entry-content p {
                text-align: justify;
            }';
        }
        $single_content_max_width = seomun_get_opt( 'single_content_max_width' );
        $single_content_max_width_page = seomun_get_page_opt( 'single_content_max_width' );
        ...
        /* Footer */
        $footer_top_link_color = seomun_get_page_opt( 'footer_top_link_color' );
        ...
        /* Custom Css */
        $custom_css = seomun_get_opt( 'site_css' );
